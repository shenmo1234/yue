...
const CACHE = 'mochi-mtwp5nm3';
const BUILD_INFO = '部署于 2026-09-11 16:31';
...
self.addEventListener('fetch', (e) => {
  const req = e.request;
  // v3.28.x #220：一起看·系统「分享到本站」收件——其他 App 分享视频（文件）时，
  // Chromium share_target 会 POST multipart 到 action=./video-share。此处拦截落库后
  // 302 回主页 ?vd=<ts>，页面 video.js 读键即播。非视频/超大走带提示的跳回。
  if (req.method === 'POST') {
    const _su = new URL(req.url);
    if (_su.pathname.indexOf('/video-share') !== -1) {
      e.respondWith((async () => {
        try {
          ...
          if (!file || !/^video\//i.test(file.type || '')) {
            return Response.redirect(new URL('./?vd=notvideo', self.location.origin).toString(), 302);
          }
          if (file.size > SHARE_MAX_BYTES) {
            return Response.redirect(new URL('./?vd=toobig', self.location.origin).toString(), 302);
          }
          const ts = String(Date.now());
          await psyncIdbSet(SHARE_INBOX_KEY, { ts: ts, title: title || file.name || '', file: file });
          return Response.redirect(new URL('./?vd=' + ts, self.location.origin).toString(), 302);
        } catch (err) {
          return Response.redirect(new URL('./?vd=fail', self.location.origin).toString(), 302);
        }
      })());
      return;
    }
    return; // 其它 POST 一律不接管
  }
...
// v3.28.x #220：一起看·系统「分享视频」收件箱（manifest share_target POST → 本键落 blob，
// 302 回 ./?vd=<ts> 由页面 video.js 读取后自动消费删除；单键设计=同一时刻只保留一份待播）
const SHARE_INBOX_KEY = 'xy-home-v2:video-share-inbox';
const SHARE_MAX_BYTES = 400 * 1024 * 1024;
